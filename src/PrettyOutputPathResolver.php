...
class PrettyOutputPathResolver
{
    public function link($path, $name, $type, $page = 1)
    {
        if ($type === 'html' && $name === 'index') {
            if ($page > 1) {
                return  DIRECTORY_SEPARATOR . $this->trimPath($path) . DIRECTORY_SEPARATOR . $page . DIRECTORY_SEPARATOR;
            }
        ...
        if ($type === 'html' && $name !== 'index') {
            if ($page > 1) {
                return  DIRECTORY_SEPARATOR . $this->trimPath($path) . DIRECTORY_SEPARATOR . $name . DIRECTORY_SEPARATOR . $page . DIRECTORY_SEPARATOR;
            }
